...
open class TwitterPagerTabStripViewController: PagerTabStripViewController, PagerTabStripDataSource, PagerTabStripIsProgressiveDelegate {
    ...
    open func updateIndicator(for _: PagerTabStripViewController, fromIndex _: Int, toIndex _: Int) {
        fatalError()
    }
    ...
    private lazy var titleScrollView: UIScrollView = { [unowned self] in
        let titleScrollView = UIScrollView(frame: CGRect(x: 0, y: 0, width: view.bounds.width, height: 44))
    ...
    private lazy var pageControl: FXPageControl = { [unowned self] in
        ...
        pageControl.dotColor = settings.style.dotColor
        pageControl.selectedDotColor = settings.style.selectedDotColor
    ...
    private var distanceValue: CGFloat? {
        navigationController.map { $0.navigationBar.convert($0.navigationBar.center, to: titleView) }?.x
    }
}
