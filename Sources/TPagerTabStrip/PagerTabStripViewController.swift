...
open class PagerTabStripViewController: UIViewController, UIScrollViewDelegate {
    ...
    open func viewControllers(for _: PagerTabStripViewController) -> [UIViewController] {
    ...
    open func canMoveTo(index: Int) -> Bool {
        currentIndex != index && viewControllers.count > index
    }

    open func pageOffsetForChild(at index: Int) -> CGFloat {
        CGFloat(index) * containerView.bounds.width
    }

    open func offsetForChild(at index: Int) -> CGFloat {
        (CGFloat(index) * containerView.bounds.width) + ((containerView.bounds.width - view.bounds.width) * 0.5)
    }
    ...
    open func virtualPageFor(contentOffset: CGFloat) -> Int {
        Int((contentOffset + 1.5 * pageWidth) / pageWidth) - 1
    }
